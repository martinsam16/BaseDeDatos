...
SELECT CODVEN FROM VENTA ORDER BY CODVEN ASC;
...
SELECT EMPRESA.NOMEMP, EMPRESA.RUCEMP, EMPRESA.TLFEMP, EMPRESA.EMAILEMP, PERSONA.DNIPER, PERSONA.APEPER, PERSONA.NOMPER, EMPRESA.TIPEMP, DEPARTAMENTO.NOMDEP,PROVINCIA.NOMPROV,DISTRITO.NOMDIS FROM EMPRESA
INNER JOIN PERSONA ON EMPRESA.PERSONA_CODPER_REPLEG=PERSONA.CODPER
INNER JOIN DISTRITO ON EMPRESA.DISTRITO_CODDIS_DISEMP = DISTRITO.CODDIS
INNER JOIN PROVINCIA ON DISTRITO.PROVINCIA_CODPROV = PROVINCIA.CODPROV
INNER JOIN DEPARTAMENTO ON PROVINCIA.DEPARTAMENTO_CODDEP = DEPARTAMENTO.CODDEP;

SELECT VENTA.CODVEN, PERSONA.NOMPER as vendedor, PERSONA1.NOMPER as comprador, VENTA.FECVEN, VENTA.HOVEN, VENTA.TIPVEN FROM VENTA
...
SELECT VENTA_DETALLE.VENTA_CODVEN_VENVEN, VENTA_DETALLE.CODDETVEN, PRODUCTO.NOMPRO, VENTA_DETALLE.CANTPROVEN, PRODUCTO.PREPRO, VENTA_DETALLE.DSCPROVEN, VENTA_DETALLE.ESTVEN FROM VENTA_DETALLE
